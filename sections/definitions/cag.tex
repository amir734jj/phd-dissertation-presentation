...
\item Basically, we need to test if there is a cycle in dependency graph for every possible derivation, but a context-free grammar may have an infinite number of derivations
}
...
The following set of binary \customorder{} relationship: $S=\{ a \lesssim b$, $b \lesssim a$,  $b \lesssim c$, $c \lesssim c \}$ can be expressed as total order on the partition of $S$: $\{ a, b\} < \{ c \}$.
...
\note[itemize]{
\item Lastly, we \textbf{proved} in the thesis that the definition of $l$-ordered can be extended to circular attribute grammars
}
...
\note[itemize]{
\item Remote attribute grammars were introduced separately by Boyland and Hedin. Basically, it extends classical attribute grammar and introduces a set of objects and a set of fields
\item This means that we now have objects and we can pass them around by reference
\item Furthermore, it gives the user more flexibility as now we are now allowed to read and write non-local attributes as opposed to only local read and local writes in classical attribute grammar
}
...
\note[itemize]{
\item Semantic of remote attribute grammar can be defined in terms of classical attribute grammar, this process is call fiber construction
\item However, the problem is objects can be attributes and attributes can be objects so we end up with an improper attribute grammar with infinite number of attributes instances
\item Professor Boyland defined fiber approximation which can do just that in finite number of attribute instances
\item Subsequently, we can apply that process to definition of $l$-ordered for classical attribute grammar to define ordered remote attribute grammars
}
...
\item Similarly, writes to field of an object should strictly precede the read of the field of the same object
}

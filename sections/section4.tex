...
\note[itemize]{
\item First and Follow are functions used in parsing and they are recursively defined and we will use them to validate our implementation of static evaluator.
}
...
\note[itemize]{
\item These are some example of First and Follow.
}
...
\item Notice the recursive nature of the problem specifically in the last equation where we have Follow on the both sides
}
...
\note[itemize]{
\item This a definition of a context-free grammar in APS
}
...
\end{frame}
\note[itemize]{
\item This a implementation of First in APS
}
...
\note[itemize]{
\item This a implementation of Follow in APS
}
...
\note[itemize]{
    \item I am intentionally not going into details of fiber construction because it would be disservice to remote attribute grammar paper but basically
    \item Its construction that expresses the semantics of remote attribute grammars in classical terms
    \item Because objects implicitly carry separate values for the fields and it is like a rope which can be separated into the individual fibers
...
\note[itemize]{
\item Even in a remote attribute grammar that is non-circular we may have fiber cycles and its okay.
\item Fiber cycles are different between remote attribute grammar and circular remote attribute grammar because in a latter case they may carry value. 
}
...
\note[itemize]{
\item Tested the fiber cycle breaking algorithm against First and Follow example.
...
\item So we designed a replacement that is still greedy but simpler as it schedules instances as groups and uses visit markers to indicate where child visit happens and when we need to return back to parent visit form a child visit
}
...
\note[itemize]{
\item The implementation of static schedule generation is currently in-progress but very close to be done.
}
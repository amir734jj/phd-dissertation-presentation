...
\note[itemize]{
\item APS has a module that generates code in C++ and Scala but it was as a proof of concept work and it had some issue.
...
\item But in APS lattices are defined as modules with generic parameter and the module itself extends its own generic parameter and Scala does not support this feature and it resulted in generating a Scala code that does not type check
...
\note[itemize]{
\item Fixing code generation turned out to tricky because internally APS types are AST nodes
\item Creating AST nodes outside of parser is anti-pattern
\item So we introduced a concept of canonical types and signatures which substitute formals with actuals and used it in fixing the code generation.
}
...
\item And creating AST nodes outside of parser is anti-pattern
\item So we introduced a concept of canonical types and canonical signatures which substitute formals with actuals and used it in fixing the code generation.
}
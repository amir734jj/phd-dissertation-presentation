...
\note[itemize]{
\item Next, we are going to discuss SCC chunk scheduling and how its working
}
...
\item So instead of checking if an attribute is ready to be scheduled when all its dependencies are ready, we say a group of attributes is ready to be scheduled when they are all ready 
...
\end{itemize}
\item And that is the goal of SCC chunk scheduling algorithm
}
...
\note[itemize]{
\item The first step is to create chunks
...
\item These guiding dependencies ensure that visits appear in a consecutive way eventhough some may be empty
...
\note[itemize]{
 ...
 \item If parent phase or protocol inner-set at a particular index is circular then we should have a single strongly connected component containing half-left and half-right for this phase and its child visits
 \item If parent phase is non-circular then we should greedy schedule attributes
 }
...
\note[itemize]{
 ...
 \item Notice that we first find a next ready-to-schedule chunk-component, then we find a next ready-to-schedule chunk, then we apply group scheduling and then start all over
 }
